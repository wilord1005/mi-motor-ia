...
def escribir_docx(service, nombre, contenido):
    try:
        ...
        for i, p in enumerate(doc.paragraphs):
            if "APRENDIZAJE" in p.text.upper():
                np = doc.add_paragraph()
                ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                np.text = f"[{ts}] {contenido}"
                doc.paragraphs.insert(i + 4, np)
                insertado = True
                break
        if not insertado:
            doc.add_paragraph(f"\n[NUEVO] {contenido}")
...
def python_scraping(query):
    ...
    try:
        ...
        # Intentar extraer tabla de la primera URL
        if raw:
            try:
...
def mistral_ia(prompt):
    try:
        ...
        r = c.chat.complete(model="mistral-large-latest", messages=[{"role":"user","content":prompt}])
        return r.choices[0].message.content
    except Exception as e:
...
def motor_completo(comando, fase, doc_motor, datos_evento=""):
    ...
    # ---- PASO 1: TODOS BUSCAN AL MISMO TIEMPO ----
    def buscar_python():
    ...
    # ---- PASO 2: ANÁLISIS INDIVIDUAL CON DOCS MOTOR ----
    datos_previos = ""
...
Habla directamente a los otros por su nombre. Argumenta con datos reales."""
    ...
    # ---- PASO 4: DEBATE RONDA 2 — HACIA CONSENSO ----
...
Solo ceder cuando el argumento o dato lo justifica — no por cortesía."""
    ...
    # ---- PASO 5: PYTHON GENERA EL CONSENSO ----
...
[qué información pasa a F{fase[-1] if fase[-1].isdigit() else '?'} siguiente]
...
Solo hechos verificados. Nada inventado."""
    ...
    # Actualizar datos compartidos con el consenso
...
· "¿Cuánto riesgo tiene el pick de St Pat?"
    ...
    if cmd_cc:
        ...
        if fase_cmd:
            ...
            # Consulta general — responden las 3 IAs
...
# ============================================================
# TABS F0 → F6 (secciones del motor)
# ============================================================
